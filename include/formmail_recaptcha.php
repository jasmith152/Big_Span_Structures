...
/* Copyright (c) 2002 Eli Sand */
    ...
    #
		...
		# validate type="number" fields is numbers
		#
		if ((!empty($post['number_fields'])) && ($list = explode(',', $post['number_fields']))) {
			foreach ($list as $value) {
				if (!empty($value) && !is_numeric($_POST["$value"])) {
					error_msg("($value) is not a number. Please try again.", TRUE);
				}
			}
		}
